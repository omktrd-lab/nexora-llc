...
export async function POST(request) {
  ...
  try {
    ...
    const depositId = `crypto-deposit-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    console.error("Crypto deposit submission failed:", error);
    return jsonError("Could not submit deposit request.", 502);
  }
}
