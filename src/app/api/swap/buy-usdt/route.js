...
import { calculateUsdSwapQuote, getExecutableNxrPrice, NXR_PRICING } from "@/lib/nxr-pricing";
...
export async function POST(request) {
  ...
  try {
    ...
    const reference = `NXR-USDT-SWAP-${currentUser.$id}-${requestId}`;
    ...
    if (!Number.isFinite(availableUsdt) || usdtAmount > availableUsdt) {
      return jsonError(
        "The swap amount exceeds your available USDT balance.",
        400,
      );
    }
    ...
    const btc24hChangePercent = Number(process.env.NXR_BTC_24H_CHANGE_PERCENT || 0);
    ...
    const quote = await calculateUsdSwapQuote({
    ...
    const existingCostBasis = Number(currentUser.prefs?.nxrCostBasisUsd || 0);
    const nextPrefs = {
      ...(currentUser.prefs || {}),
      usdtBalance: availableUsdt - usdtAmount,
      nxrBalance: existingNxrBalance + quote.nxrReceived,
      nxrCostBasisUsd: existingCostBasis + quote.netUsdAmount,
      lastSwapAt: now,
    ...
    console.error("NXR USDT swap execution failed:", error);
    try {
